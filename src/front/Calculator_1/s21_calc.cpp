...
// my funcs:
void s21_calc::on_pushButton_clear_all_clicked() {
...
void s21_calc::click_to_operator() {
  QPushButton *button = (QPushButton *)sender();
  bool is_print = false;
  QString text = ui->label_show_field->text();
  if (text.contains("Error") || text.contains("nan") || text.contains("inf") ||
...
void s21_calc::click_to_bracket() {
  QPushButton *button = (QPushButton *)sender();
  bool is_print = false;
  QString text = ui->label_show_field->text();
  if (text.contains("Error") || text.contains("nan") || text.contains("inf") ||
...
void s21_calc::click_to_math_foo() {
  QPushButton *button = (QPushButton *)sender();
  bool is_print = true;
  QString text = ui->label_show_field->text();
  if (text.contains("Error") || text.contains("nan") || text.contains("inf") ||
...
double s21_calc::convert_x_to_y(double x, QString text) {
...
void s21_calc::on_pushButton_create_graph_clicked() {
  bool is_print = true;
  ...
  if (text.contains('x')) {
    ...
    if (!api_to_validate(cstr)) {
      ...
      center_y = s21_calc::convert_x_to_y(x_value, text);
...
void s21_calc::on_pushButton_x_clicked() {
  QString text = ui->label_show_field->text();
  if (text.contains("Error") || text.contains("nan") || text.contains("inf") ||
      text.contains('e')) {
    text.clear();
